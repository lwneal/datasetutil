...
def main():
    ...
    for split in range(5):
        np.random.shuffle(animal_labels)
        np.random.shuffle(object_labels)

        # From animals, select 10, 20, ... labels
        with open(os.path.join(DATA_DIR, 'cifar100-animals-10-split_{}.dataset'.format(split)), 'w') as fp:
            for e in tqdm(e for e in examples if e['label'] in animal_labels[:10]):
                fp.write(json.dumps(e) + '\n')
        with open(os.path.join(DATA_DIR, 'cifar100-animals-20-split_{}.dataset'.format(split)), 'w') as fp:
            for e in tqdm(e for e in examples if e['label'] in animal_labels[:20]):
                fp.write(json.dumps(e) + '\n')
        with open(os.path.join(DATA_DIR, 'cifar100-animals-50-split_{}.dataset'.format(split)), 'w') as fp:
            for e in tqdm(e for e in examples if e['label'] in animal_labels[:50]):
                fp.write(json.dumps(e) + '\n')

        # From non-animals, select 10, 20, ... labels
        with open(os.path.join(DATA_DIR, 'cifar100-not-animals-10-split_{}.dataset'.format(split)), 'w') as fp:
            for e in tqdm(e for e in examples if e['label'] in object_labels[:10]):
                fp.write(json.dumps(e) + '\n')
        with open(os.path.join(DATA_DIR, 'cifar100-not-animals-20-split_{}.dataset'.format(split)), 'w') as fp:
            for e in tqdm(e for e in examples if e['label'] in object_labels[:20]):
                fp.write(json.dumps(e) + '\n')
        with open(os.path.join(DATA_DIR, 'cifar100-not-animals-50-split_{}.dataset'.format(split)), 'w') as fp:
